...
scores <- read_xlsx('Knights 2024 Scoring .xlsx', sheet = "Goals For")
...
schedule <- 
  read_xlsx('Knights 2024 Scoring .xlsx', sheet = "Schedule")
...
games_played <- read_xlsx('Knights Attendance and Game Locations.xlsx', sheet = "Attendance") 
games_played <- games_played[4:23,2:(2+games_played_N)]
...
goals_to_viz <- 
  left_join(goals_to_viz, 
            scores %>% filter(Action == "Assist") %>% 
              select(Date, Opponent, Score, Player, `Pass Type`, `Loc Y`, `Loc X`) %>% 
...
output$message <- 
  renderText(
    ...
    case_when(
      goals_to_viz %>% filter(handle == input$select_goal) %>% select(Position) %>% unlist() == "Outside the Box" ~ 
        "BOOOOOOOM. A banger from downtown", 
      T ~ "Great goal!"
    ) %>% print()  
  )

# output$message <- renderText(print("HELLO"))
textOutput('message')
...
output$message_goal <- renderText(
  goals_to_viz %>% filter(handle == input$select_goal) %>% select(Message) %>% unlist()
)

textOutput('message_goal')
```
